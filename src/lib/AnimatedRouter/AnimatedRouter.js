        ...
        return (
          (_ret = ((_temp = ((_this = possibleConstructorReturn(
            ...
            (_ref =
              ...
              Object.getPrototypeOf(AnimatedRouter)).call.apply(
              _ref,
              [this].concat(args)
            )
          )),
          ...
          (_this.onEnter = function(node) {
            _this.inTransition ||
              _this.setInTransition((_this.inTransition = true))
            _this.lastTransitionNode = node
          }),
          (_this.onEntered = function(node) {
            if (_this.lastTransitionNode === node) {
              _this.inTransition &&
                _this.setInTransition((_this.inTransition = false))
            }
            ...
            if (node) {
              ...
              node.className = node.className
                ...
                .filter(function(name) {
                  return !/-(?:forward|backward)-(?:enter|exit)(?:-active)?$/.test(
                    name
                  )
                })
                .join(' ')
              ...
              Object.assign(
                {
                  ...
                  childFactory: function childFactory(child) {
                    var classNames =
                      prefix +
                      '-' +
                      (isHistoryPush(location, child.props.in)
                        ? 'forward'
                        : 'backward')
