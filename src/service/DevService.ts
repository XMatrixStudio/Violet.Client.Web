...
const DevService = {
  ...
  newOrg: async (data: PostOrgs.ReqBody) => {
    const res = await Request.post(BASE_ORG, data)
    return res
  },
  // 通过ID获取组织信息
  getOrgInfoByID: async (id: string, all = false) => {
    const res = await Request.get<GetOrgsByExtId.ResBody>(BASE_ORG + '/+' + id, {
      params: { all: all }
    })
    return res
  },
  ...
  getOrgInfoByName: async (name: string, all = false) => {
    const res = await Request.get<GetOrgsByExtId.ResBody>(BASE_ORG + '/' + name, {
      params: { all: all }
    })
    return res
  },
  ...
  getOrgApps: async (page: number, limit: number, id: string) => {
    ...
    const res = await Request.get<GetOrgsByIdApps.ResBody>(
      BASE_ORG + '/' + id + '/apps',
      {
        params: params
      }
  ...
  getOrgMembers: async (id: string, page = 1, limit = 10) => {
    ...
    const res = await Request.get<GetOrgsByIdMembers.ResBody>(
      BASE_ORG + '/' + id + '/members',
      {
        params: params
      }
  ...
  addOrgMember: async (orgID: string, userID: string) => {
    ...
    const res = await Request.post(BASE_ORG + '/' + orgID + '/members', req)
    return res
  },
  // 删除组织成员
  ...
  editOrgMember: async (orgID: string, userID: string, role: 0 | 1) => {
    ...
    const res = await Request.put(BASE_ORG + '/' + orgID + '/members', req)
    return res
  },
  ...
  improveAppCount: async (data: PostUsersRequestsApps.ReqBody) => {
    const res = await Request.post(BASE_USER + '/requests/apps', data)
    return res
  },
  // 提高开发者组织上限
  // repeat_request - 重复申请
  improveOrgCount: async (data: PostUsersRequestsOrgs.ReqBody) => {
    const res = await Request.post(BASE_USER + '/requests/orgs', data)
    return res
  },
  // 获取开发者个人应用信息
  getDevApps: async (page: number, limit: number, user = 'me') => {
    ...
    const res = await Request.get<GetUsersByUidApps.ResBody>(
      BASE_USER + '/' + user + '/apps',
      { params: params }
    )
    return res
  ...
  getDevOrgs: async (page: number, limit: number, user = 'me') => {
    ...
    const res = await Request.get<GetUsersByUidOrgs.ResBody>(
      BASE_USER + '/' + user + '/orgs',
      {
        params: params
      }
  ...
  newApp: async (req: PostApps.ReqBody) => {
    const res = await Request.post(BASE_APP, req)
    return res
  },
  // 更新应用信息
  updateApp: async (req: PatchAppsById.ReqBody, id: string) => {
    const res = await Request.patch(BASE_APP + '/' + id, req)
    return res
  },
  // 通过 ID 获取应用信息
  getAppInfoById: async (id: string, all?: boolean) => {
    const res = await Request.get<GetAppsByExtId.ResBody>(BASE_APP + '/+' + id, {
      params: {
        all: all
      }
    })
  ...
  getAppInfoByName: async (name: string, all?: boolean) => {
    const res = await Request.get<GetAppsByExtId.ResBody>(BASE_APP + '/' + name, {
      params: {
        all: all
      }
    })
