...
const UserService = {
  ...
  fetchUserInfo: async (store: IStore) => {
    try {
      const res = await service.get<GetUsersByExtUid.ResBody>('/api/i/users/me')
      store.user = res.data
    } catch (_) {
      store.user = null
    }
  ...
  UpdateDevInfo: async (req: PutUsersDev.ReqBody) => {
    const res = await service.put('/api/i/users/dev', req)
    return res
  },
  // 授权
  Auth: async (req: PostUsersAuths.ReqBody) => {
    const res = await service.post('/api/i/users/auths', req)
    return res
  },
  // 获取授权信息
  GetAuthByID: async (id: string, url: string) => {
    ...
    const res = await service.get<GetUsersAuthsByAppId.ResBody>(
  ...
  GetAuths: async (page = 1, limit = 10) => {
    ...
    const res = await service.get<GetUsersAuths.ResBody>('/api/i/users/auths', {
      params: req
    })
    return res
  },
  ...
  RemoveAuth: async (id: string) => {
    const res = await service.delete('/api/i/users/auths/' + id)
    return res
  },
  // 搜索用户
  SearchUser: async (key: string, page = 1, limit = 10) => {
    ...
    const res = await service.get<GetUsers.ResBody>('/api/i/users', {
      params: req
    })
    return res
  },
  // 获取用户待申请列表
  GetRequests: async () => {
    const res = await service.get<GetUsersRequests.ResBody>(
  ...
  UpdateLevel: async (req: PostUsersRequestsLevels.ReqBody) => {
    const res = await service.post('/api/i/users/requests/levels', req)
    return res
  },
  // 获取指定用户信息
  GetUserInfo: async (name: string) => {
    const res = await service.get<GetUsersByExtUid.ResBody>(
      '/api/i/users/' + name
    )
    return res
  },
  ...
  GetUserInfoByID: async (id: string) => {
    const res = await service.get<GetUsersByExtUid.ResBody>('/api/i/users/+' + id)
    return res
  },
  // 修改信息
  UpdateInfo: async (req: PatchUsers.ReqBody) => {
    ...
    const res = await service.patch('/api/i/users', req)
    return res
  },
  // 登陆
  Login: async (account: string, password: string, remember: boolean) => {
    ...
    const res = await service.post('/api/i/users/session', req)
    return res
  },
  // 注册
  Register: async (userName: string, nickname: string, password: string) => {
    ...
    const res = await service.post('/api/i/users', req)
    return res
  },
  // 验证邮箱/手机 - 注册
  Valid: async (
    ...
    const res = await service.put(url, req)
    return res
  },
  // 验证手机/邮箱 - 重置密码
  ResetPassword: async (password: string) => {
    const res = await service.post('/api/i/users/reset', {
      password: createHash('sha512')
        .update(password)
        .digest('hex')
    })
  ...
  GetValid: async (
    ...
    const res = await service.post(url, req)
    return res
  },
  // 退出登陆
  Logout: async () => {
    await service.delete('/api/i/users/session')
  }
}
